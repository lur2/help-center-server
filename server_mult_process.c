...
int read_line(char *buf, int *fd_ptr){
    ...
    int nbytes;
    while ((nbytes = read(*fd_ptr, after, room)) > 0) {
        inbuf += nbytes;
 ...
 */
int read_from(int* hc_fd, int client_index, struct sockname *usernames, fd_set *listen_fds) {
    ...
    int len_next = -1;
    if ((len_next = read_line(buf, &(usernames[client_index].sock_fd))) < 0){
        // closed
        return fd;
    } else {
        // check buf content
        ...
        if (c2 == 1 && c3 == 0){
            dprintf(hc_fd[1], "next");
        
        } else if (c1 == 0) { 
            //TODO: write to Help Centre pipe
            if (c2 == 1){
                dprintf(hc_fd[1], "print_full_queue");
            } else if (c2 == 2){
                dprintf(hc_fd[1], "print_currently_serving");
            }
            //TODO: read from Help Centre
            char output[BUF_SIZE];
            int num_read = read(hc_fd[0], &output, BUF_SIZE);
            buf[num_read] = '\0';
            if (num_read == 0)  {
                perror("read from helpcentre");
            }
            
            //TODO: output to client
            if (write(fd, output, strlen(buf)) != strlen(buf)){
                // closed
                return fd;
            }
        } else {
 ...
 * Return the fd if it is closed or 0 otherwise.
 */
int read_type(int* hc_fd, int client_index, struct sockname *usernames) {
    // check
    if (usernames[client_index].username == NULL) {
    ...
    if ((len_next = read_line(buf, &(usernames[client_index].sock_fd))) < 0){
        ...
        if (strlen(buf) != 1) {
            ...
            dprintf(hc_fd[1], "add_ta %s", usernames[client_index].username);
 ...
 * Return the fd if it is closed or 0 otherwise.
 */
int read_course(int *hc_fd, int client_index, struct sockname *usernames) {
    // check
    if (usernames[client_index].username == NULL) {
        perror("client unnamed.");
        exit(1);
    ...
    if ((len_next = read_line(buf, &(usernames[client_index].sock_fd))) < 0){
        ...
        if (strlen(buf) != 6){
            ...
            dprintf(hc_fd[1], "add_student %s %s", usernames[client_index].username, buf);
        // instruction
...
int main(void) {
    ...
    if ((pid = fork()) == 0){
        // reset stdin
        if ((dup2(hc_fd[0], fileno(stdin))) == -1) {
            //assert
            perror("helpcentre reset stdin");
            exit(1);
        }
        // reset stdout
        if ((dup2(hc_fd[1], fileno(stdout))) == -1) {
            // assert
            perror("helpcentre reset stdout");
            exit(1);
        }
        // close fd[0]
        close(hc_fd[0]);
        // start helpcentre
        execl("./helpcentre", "helpcentre", (char *)NULL);
        perror("exec helpcentre");
        exit(1);
    } else if (pid > 0) {
    ...
    for (int index = 0; index < MAX_CONNECTIONS; index++) {
        ...
        usernames[index].buf = malloc(sizeof(char)*BUF_SIZE);
    }
    ...
    while (1) {
        ...
        for (int index = 0; index < MAX_CONNECTIONS; index++) {
            if (usernames[index].sock_fd > -1 && FD_ISSET(usernames[index].sock_fd, &listen_fds)) {
                ...
                // TA / Student
                    read_type(hc_fd, index, usernames);
                } else if (usernames[index].type == 0){
                // Student course
                    read_course(hc_fd, index, usernames);
                } else {
                    // Note: never reduces max_fd
                    int client_closed = read_from(hc_fd, index, usernames, &listen_fds);
                    if (client_closed > 0) {
