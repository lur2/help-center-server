 ...
 * the student struct.
 */
int add_student(Student **stu_list_ptr, int fd, char *student_name, char *course_code,
...
char *print_currently_serving(Ta *ta_list) {
    if (ta_list == NULL) {
        char *nullline;
        asprintf(&nullline,"No TAs are currently working.\r\n");
        return nullline;
    }
    Ta *head = ta_list;
    int length = 0;
    char *buf;
    // first loop: count output length
    while (head != NULL) {
