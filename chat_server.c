...
int read_from(int client_index, struct sockname *usernames, fd_set *listen_fds) {
    ...
    for (int i = 0; i < MAX_CONNECTIONS; i++){
        if (usernames[i].sock_fd != -1){ 
            int this_fd = usernames[i].sock_fd;
            if (write(this_fd, buf, strlen(buf)) != strlen(buf)) {
            return fd;
            }
...
int read_name(int client_index, struct sockname *usernames) {
    ...
    char* name = malloc(sizeof(char)*num_read);
    strncpy(name, buf, num_read);
